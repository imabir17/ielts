  ...
  return (
    <div className="flex h-screen bg-[var(--paper)]">
      ...
      <div className="flex-1 flex flex-col font-sans h-screen overflow-hidden">
        <header className="h-16 bg-white border-b border-[var(--line)] px-8 flex items-center shrink-0">
          <button onClick={() => router.push('/org/students')} className="mr-4 text-[var(--ink-soft)] hover:text-[var(--ink)] transition-colors">
            <ArrowLeft className="w-5 h-5" />
          </button>
          <div>
            <h1 className="font-display text-[20px] text-[var(--ink)] leading-tight">{student.name}'s Profile</h1>
            <span className="text-[12px] text-[var(--ink-soft)]">Student ID: {student.studentId}</span>
          </div>
        </header>
