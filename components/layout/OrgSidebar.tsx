...
import { LayoutDashboard, Users, Send, Award, Mic, GraduationCap, AlertTriangle } from 'lucide-react';
...
export function OrgSidebar() {
  ...
  const myOrg = tenants.find(o => o.id === (isTeacher ? currentUser?.orgId : currentUser?.id)) || (currentUser?.role === 'tenant' ? currentUser : tenants[0]);
  ...
  const navItems = isTeacher
    ...
    : [
        ...
        { name: 'Teachers & Examiners', href: '/org/teachers', icon: GraduationCap },
  ...
  return (
    <aside className="sidebar shrink-0">
      <div className="side-brand">
        <div className="side-mark" style={{ borderColor: isTeacher ? 'var(--gold)' : 'var(--forest)', color: isTeacher ? 'var(--gold)' : 'var(--forest)' }}>
          {isTeacher ? 'E' : (myOrg?.name ? myOrg.name.charAt(0).toUpperCase() : 'T')}
        </div>
        <div>
          <div className="side-brand-name truncate max-w-[140px]">
            {isTeacher ? (currentUser?.name || 'Faculty Examiner') : (myOrg?.name || 'Apex Academy')}
          </div>
          <div className="side-brand-role">
            {isTeacher ? 'Certified Examiner' : `${quota.tierName} Center`}
          </div>
        </div>
      </div>

      <div className="side-section-label">
        {isTeacher ? 'Examiner Portal' : 'Tenant Portal'}
      </div>
      ...
      {/* Mini Quota Tracker in Sidebar (Hidden for Teachers) */}
      {!isTeacher && (
